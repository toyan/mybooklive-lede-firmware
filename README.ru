...
Для установки необходим компьюьер с установленной Linux-системой, или загружаемой в внешнего носителя системой Linux типа Live-CD, Live-USB и т.д.
...
Если вы загрузились в систему с флешки, тогда и она будет иметь какое-то своё имя, и тогда ваш целевой диск может иметь имя 'sdc'.
Скрипт выводит информацию о подключенных дисках, и по умолчанию выбирает диск с самой последней буквой из имеющищихся в системе.

Вам нужно, исходя из выведенной схемы подключенных дисков, правильно выбрать целевой для записи прошивки и указать в ответ на запрос, например 'sdc'.
...
После записи прошивки, подключите диск к MBL, подключите питание, и дождитесь, пока промигается зелёный светодиод и замигает синий.
...
Если же вам необходимо задать другой статический адрес, то это можно сделать сразу после записи прошивки, перед отключением диска:
...
Чтобы дополнительно добавить пользователя для доступа к диску по сети, нужно добавить его сначала как пользователя системы
командой "adduser <имя>", затем добавить его в пользователи самбы: smbpasswd <имя>.
...
По умолчанию там всё настроено для нормальной работы. Если же вам нужно внести измененения, после "Сохранить и применить"
просто перезагрузите устройство командой "reboot" в терминале или по питанию.
